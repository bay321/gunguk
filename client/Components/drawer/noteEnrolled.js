...
export default function noteEnrolled({ navigation }) {
  ...
  // useEffect(() => {
    useEffect(() => {
      // let mounted = true;
    const response = axios.get('http://172.16.129.50:3001/users/enrolled')
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ width: win.width / 2, height: win.height / 20, marginLeft: win.width / 20, flexDirection: "row", marginTop: win.height / 40, justifyContent: "center" }}>
        <Pressable  onPress={() => (console.log("hi"))} style={styles.pushed}>
          <Text>재학생(기수별)</Text>
        </Pressable>
        <Pressable onPress={() => navigation.replace("noteGraduated")} style={styles.unPushed}>
          <Text>졸업생</Text>
        </Pressable>
